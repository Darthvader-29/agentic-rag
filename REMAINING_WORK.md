...
- **Status:** DONE — 117513a (+1ef0764 overseer E2E fix) (track T7)
...
- **Status:** DONE — f793698 + 26298d9 (track T7)
...
- R20/R21/R29 — a17928a, 117513a (+1ef0764 overseer E2E fix), f793698+26298d9 — [PARALLEL track T7] CI gating, E2E, cleanup. **R20**: the frontend `quality` CI job now runs `npm run test` (vitest) between typecheck and build, so the ~50 vitest files gate PRs (CODE_REVIEW H-F8); used `npm run test` NOT the queue's buggy `-- run` (which would pass a stray name-filter → "no test files"). **R21**: new Playwright E2E (`e2e/chat.e2e.ts` + `playwright.config.ts`) covering the M5 core flow (load → send → blocking assistant reply + RAG route badge → upload → theme toggle → reset) against a `page.route()`-stubbed backend (no real server/MSW); the `.e2e.ts` suffix keeps it out of the vitest sweep; the webServer builds+serves the prod app with streaming/auth/BYOK OFF; wired into CI as a second `e2e` job (`needs: quality`, installs chromium). Overseer ran it LIVE (chromium): caught a strict-mode violation — the `/notes\.txt/` upload assertion matched both the in-chat confirmation AND a transient toast — and narrowed it to `/notes\.txt.*queued for ingestion/i`; re-ran **1 passed (33.8s)**. **R29**: removed the dead linear-flow helpers (`generation.synthesize`/`generate_final_response`, `router.route_query`) + their orphaned tests — KEPT `build_synthesis_query`/`decide_agentic_route` (used by `agents/nodes.py`); dropped the unused `UPLOADTHING_API_KEY` setting + its test refs; deleted empty `frontend/GEMINI.md`; reconciled storage-backend docs (S3 → S3-compatible MinIO/B2). Independently verified: grep confirms NO live caller of the removed helpers (only historical docs + test comments reference them). Backend ruff+format+mypy clean, pytest green except the 5 PRE-EXISTING `test_config` reds. Frontend lint+vitest **331/331**+build clean; typecheck +0 (3 pre-existing); format:check is CI-clean (the local 151-file failure is a Windows CRLF artifact — repo stores 100% LF, 0 `i/crlf`). NOTE: `.env.example` still lists UPLOADTHING (cosmetic doc-only); frontend CI lives at `frontend/.github` per the pre-existing repo convention. INCIDENT: a backgrounded T6-worktree cleanup followed a `node_modules` symlink-to-main and ate the main `node_modules` mid-run → restored with `npm ci` (no source/git impact); saved as a memory.
...
- R05 — 9e1f543 — rate-limited every auth endpoint (login/register/guest/refresh/logout/upgrade) via a **router-level dependency** `enforce_auth_rate_limit` (`auth/dependencies.py`): per-IP fixed-window counter (`AUTH_RATE_LIMIT_MAX=10` / `_WINDOW_SECONDS=60`), armed atomically (`SET 0 EX NX → INCR`) like freemium, **fails open** on a Redis outage, raises a clean `{detail}` 429. Chose a router-level dependency over slowapi `@limiter.limit` **specifically because every auth test calls the handlers DIRECTLY** — a decorator needs `request: Request` on each handler and would break ~20 direct calls; the router dep runs on HTTP only, so all direct-call tests are untouched (and `{detail}` already aligns with R27, vs slowapi's `{error}`). New `test/test_auth_rate_limit.py` (4): under-limit→429, per-IP isolation, fail-open, router-wiring. Full backend suite green except the 5 PRE-EXISTING `test_config` reds (proven unrelated); ruff+mypy clean. NOTE: one shared per-IP limit across all auth routes; keyed on `request.client.host` (needs uvicorn `--proxy-headers` behind a LB — a separate CODE_REVIEW item); per-route tuning (tighter login, looser guest) is a possible refinement.
- R04 — e0e62df — completed the auth-hardening spine on R03's jti denylist: (1) refresh-token **ROTATION** — `/refresh` consumes (denylists via `revoke_token`) the presented token before minting a fresh pair, so a refresh token is single-use and a replay is rejected (401); the frontend already persists the rotated pair, so no client change. (2) **aud/iss** — tokens carry audience+issuer (`config.JWT_AUDIENCE`/`JWT_ISSUER`, with defaults) and `decode_token` validates them, so a token minted for another aud/iss is rejected. New tests: rotation→reuse-rejected (`test_auth_router`); wrong-aud, wrong-iss, and tokens-carry-aud/iss (`test_auth_security`). Full backend suite green **except** the 5 PRE-EXISTING `test_config` env-validation reds (proven unrelated via stash); ruff+mypy clean. DEFERRED (noted): refresh-time DB re-derivation (a deleted user is already rejected at `get_current_user`) and reuse-detection family revocation — both layer on this jti foundation.
